    ...
    >
      <div className="mb-3 flex items-baseline justify-between gap-3">
        <h3 className="font-[family-name:var(--font-display)] text-[1.05rem] font-bold tracking-tight">
          购物车
          <span className="ml-2 text-[0.78rem] font-medium text-ink-mute">{items.length}</span>
        </h3>
      ...
      {!items.length ? (
        ...
        <ul className="max-h-[min(46vh,320px)] space-y-0 overflow-y-auto">
          {items.map((item, i) => (
            <li key={`${item.id}-${i}`} className="flex items-start justify-between gap-3 py-2.5 first:pt-0 last:pb-0">
              ...
              <div className="shrink-0 pt-0.5 text-[0.9rem] font-semibold text-ink">¥{item.price}</div>
            </li>
          ))}
        </ul>
      )}
