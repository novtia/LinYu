...
import type { EzpayConfig, PaymentChannel, PaymentProvider } from '../../types'
...
export function PaymentChannelFormPage() {
  ...
  const [config, setConfig] = useState<EzpayConfig>(DEFAULT_EZPAY)
  ...
  async function save() {
    ...
    if (provider === 'ezpay' && !config.pid.trim()) {
      showToast('请填写商户 ID (pid)')
      return
    }
    if (provider === 'ezpay' && !config.key.trim()) {
      showToast('请填写商户密钥')
      return
    }
    setSaving(true)
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="overflow-hidden rounded-[18px] border border-[var(--line)] bg-white">
        <div className="grid gap-4 border-b border-[var(--line)] p-[18px] md:grid-cols-2">
          <Field label="渠道名称" value={name} onChange={setName} placeholder="例如：易支付主通道" />
          ...
          <>
            ...
            <div className="grid gap-4 border-b border-[var(--line)] p-[18px] md:grid-cols-2">
              ...
              <Field
                label="商户 ID (pid)"
                value={config.pid}
                onChange={(v) => setConfig({ ...config, pid: v })}
            ...
            <SwitchRow
              title="支付宝 alipay"
              desc="type=alipay"
              on={config.methods.alipay}
              onToggle={() =>
                setConfig({
                  ...config,
                  methods: { ...config.methods, alipay: !config.methods.alipay },
                })
              }
            />
            <SwitchRow
              title="微信支付 wxpay"
              desc="type=wxpay"
              on={config.methods.wxpay}
              onToggle={() =>
                setConfig({
                  ...config,
                  methods: { ...config.methods, wxpay: !config.methods.wxpay },
                })
              }
            />
            <SwitchRow
              title="QQ 钱包 qqpay"
              desc="type=qqpay"
              on={config.methods.qqpay}
              onToggle={() =>
                setConfig({
                  ...config,
                  methods: { ...config.methods, qqpay: !config.methods.qqpay },
                })
              }
            />

            <div className="border-t border-[var(--line)] bg-[rgba(232,241,238,.45)] px-[18px] py-3.5 text-[0.8rem] leading-relaxed text-ink-soft">
              签名规则：参数按 ASCII 排序拼接为 <code className="font-[family-name:var(--font-mono)]">key=value</code>
              链（除去 sign / sign_type），末尾追加商户密钥后 MD5。参考文档与{' '}
              <code className="font-[family-name:var(--font-mono)]">pay.txt</code> 示例。
            </div>
          </>
        )}
